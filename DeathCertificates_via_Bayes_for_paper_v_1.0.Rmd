...
testingGrid <-  expand.grid(replicate=1:10, nSamples=nSamples,
                            ageGroup=c('Adult'), gender=0:1,  nGrams=2,
    					priorMethod=c('Good-Turing'), constant=.Machine$double.eps,
              useQ2=c(FALSE), columnNormalize=c(TRUE,FALSE), useCausadef=c(TRUE,FALSE),
							stringsAsFactors=FALSE)

testingGrid <- cbind(testingGrid, csmfAccuracy=NA, meanCCC=NA, meanpCCC1=NA)
...
#  if (testingGrid[i,'gender'] == 0) dat1$gender = 'Glort'
  ...
  b=ddply(retList$csmf2, .(cause), summarize, mean(CCC[is.finite(CCC)]))
  a=ddply(retList$csmf,  .(cause), summarize, mean(CCC[is.finite(CCC)]))
  temp = cbind(a[,1:2],b[,2])
  colnames(temp) <- c('cause', 'csmf', 'csmf2')
  temp$cause = factor(temp$cause)
  plot(xyplot(csmf2 ~ csmf, groups=reorder(cause, -csmf2, mean), data=temp, type=c('p','g'), 
  ...
  temp <- ddply(retList$csmf, .(cause), summarize, mean(CCC[is.finite(CCC)]))
  colnames(temp) <- c('cause', 'meanCCC')
  print(kable(temp, format='markdown'))
  cat('\n\n****\n\n')
  
  temp <- with(retList$csmf, mean(CCC[is.finite(CCC)]))
  testingGrid[i, 'meanCCC'] <- temp
  cat('The overall mean is:   ', temp, '   \n')
  cat('\n\n****\n\n')
  ...
  cat('\n\n****\n\n')  
  testingGrid[i, 'meanpCCC1'] <- mean(retList$pCCC[,2], na.rm=TRUE)
